...
try {
    ...
    if ($AFASEmployeeRows.count -eq 0) {
        $OutputContext.AuditLogs.add([PSCustomObject]@{
                ...
                Message = "Could not find employee record in GET-Connector '$($ActionContext.Configuration.GETConnector)' based on correlation request $($CorrelationRequestLog), so the data is already deleted."
                IsError = $false
            }
        )
    }
